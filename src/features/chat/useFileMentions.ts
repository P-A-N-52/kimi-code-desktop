...
const WORKSPACE_STALE_MS = 120_000;
...
const shouldIgnoreDirectory = (name: string): boolean =>
...
const listWorkspaceDirectory = async ({
  ...
  const basePath = path && path !== "." ? normalizePath(path) : ".";
  ...
  for (const entry of entries) {
    ...
    if (entry.type === "directory" && shouldIgnoreDirectory(entry.name)) {
      continue;
    }
    const fullPath =
      basePath === "." ? entry.name : `${normalizePath(basePath)}/${entry.name}`;
    files.push({
      path: fullPath,
      kind: entry.type,
      size: entry.size,
...
const toWorkspaceOptions = (files: WorkspaceFile[]): MentionOptionBase[] =>
  files.map((file) => {
    ...
    const baseLabel = segments.at(-1) ?? file.path;
    const label = file.kind === "directory" ? `${baseLabel}/` : baseLabel;
    const insertValue =
      file.kind === "directory" ? `${file.path}/` : file.path;
    return {
      id: `workspace-${file.kind}-${file.path}`,
...
export const useFileMentions = ({
  ...
  const directoryCacheRef = useRef(
    new Map<string, { files: WorkspaceFile[]; timestamp: number }>(),
  );
  ...
  const mergedWorkspaceFiles = useMemo(() => {
    ...
    const existing = new Set(workspaceFiles.map((f) => `${f.kind}:${f.path}`));
    const extra = directoryFiles.filter(
      (f) => !existing.has(`${f.kind}:${f.path}`),
    );
  ...
  const loadDirectoryFiles = useCallback(
    async (dirPath?: string): Promise<WorkspaceFile[]> => {
      if (!(sessionId && listDirectory)) {
        return [];
      }
      const normalizedPath =
        dirPath && dirPath !== "." ? normalizePath(dirPath) : ".";
      const cached = directoryCacheRef.current.get(normalizedPath);
      if (cached && Date.now() - cached.timestamp < WORKSPACE_STALE_MS) {
        return cached.files;
      }
      const files = await listWorkspaceDirectory({
        sessionId,
        listDirectory,
        path: normalizedPath,
      });
      directoryCacheRef.current.set(normalizedPath, {
        files,
        timestamp: Date.now(),
      });
      return files;
  ...
  }, [sessionId, listDirectory, loadDirectoryFiles]);
  ...
  }, [range?.query, sessionId, listDirectory, loadDirectoryFiles]);
  ...
  const selectOption = useCallback(
    (option?: MentionOption) => {
      ...
      const isDirectory = target.meta?.isDirectory === true;
      const needsSpace =
        isDirectory || !(after.length === 0 || NON_WHITESPACE_START.test(after))
          ? ""
          : " ";
  ...
  return {
    ...
    workspaceError,
    retryWorkspace,
    workspaceFileCount: workspaceFiles.filter((file) => file.kind === "file").length,
  };
};
