...
import {
  deriveWorkspacePendingRequestCounts,
  type WorkbenchStreamSnapshot,
} from "@/features/workbench/workspace-panel";
import { useQueueStore } from "./queue-store";
import { ChatWorkspace } from "./chat";
import { deriveActivityStatus } from "./components/activity-status-indicator";
...
type ChatWorkspaceContainerProps = {
  ...
  layoutMode?: "single" | "workbench";
  onStreamSnapshotChange?: (snapshot: WorkbenchStreamSnapshot) => void;
};
...
export function ChatWorkspaceContainer({
  ...
  const streamSnapshot = useMemo<WorkbenchStreamSnapshot>(
    () => ({
      chatStatus: status,
      isConnected: isStreamConnected,
      isReplayingHistory,
  ...
  useEffect(() => {
    onStreamSnapshotChange?.(streamSnapshot);
  }, [onStreamSnapshotChange, streamSnapshot]);
