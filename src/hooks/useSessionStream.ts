...
export function useSessionStream(
  ...
  const promptTimingRef = useRef<PromptTiming | null>(null);
  const preserveMessagesOnConnectRef = useRef(false);
  const skipReplayOnConnectRef = useRef(false);
  ...
  const inlineThinkParserRef = useRef<InlineThinkParseState>({
    inThink: false,
    buffer: "",
  });
  ...
  const scheduleBufferedStreamUpdate = useCallback(() => {
  ...
  const flushBufferedStreamUpdate = useCallback(() => {
    if (streamUpdateFrameRef.current !== null) {
  ...
  const appendThinkingContent = useCallback(
    (delta: string, isReplay: boolean) => {
      if (!delta) {
        return;
      }
  ...
  const appendTextContent = useCallback(
    (delta: string, isReplay: boolean) => {
      if (!delta) {
        return;
      }
  ...
  const appendInlineThinkSegments = useCallback(
    (segments: InlineThinkSegment[], isReplay: boolean) => {
    ...
    [appendThinkingContent, appendTextContent],
  );
  ...
  const sealOpenStreamBlocks = useCallback(() => {
  ...
  }, [flushBufferedStreamUpdate, setMessages]);
  ...
  }, [flushBufferedStreamUpdate, flushInlineThinkBuffer]);
  ...
  const showStepRetryStatus = useCallback(
    ...
    [clearStepRetryStatus, getNextMessageId, setMessages],
  );
  ...
  const sendInitialize = useCallback(async (ws: StreamConnection) => {
    const id = uuidV4();
  ...
  const finishConnection = useCallback(
    (connection: StreamConnection, code?: number, reason?: string) => {
  ...
  const sendSetPermissionMode = useCallback(
  ...
  }, [flushPendingModeUpdates, status]);
  ...
  useLayoutEffect(() => {
    ...
    const startSession = async () => {
      ...
      if (isTauri() && !autoConnectRef.current) {
        ...
        try {
          const historyMessages = await replaySessionHistory(sessionId);
          if (cancelled) {
            return;
          }
