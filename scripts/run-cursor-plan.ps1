...
if ($RunId -notmatch '^[A-Za-z0-9_.-]+$') {
    throw "RunId may only contain letters, numbers, dots, underscores, and hyphens."
}
...
$stdoutPath = Join-Path $tmpDir "$RunId.out.jsonl"
$stderrPath = Join-Path $tmpDir "$RunId.err.log"
$exitCodePath = Join-Path $tmpDir "$RunId.exitcode.txt"
$invokeScriptPath = Join-Path $tmpDir "$RunId.invoke.ps1"
